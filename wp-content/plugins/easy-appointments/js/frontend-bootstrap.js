;(function ( $, window, document, undefined ) {
	...
	$.extend(Plugin.prototype, {
		...
		init: function () {
			...
			this.$element.find('.ea-bootstrap').on('click', '.time-value', function(element) {
				...
				if (ea_settings['pre.reservation'] === '1') {
					...
					booking_data.time = plugin.$element.find('.selected-time').text();
				...
				plugin.loadPreviusFormData(plugin.$element.find('form'));
			});
			...
			$.get(ea_ajaxurl, options, function(response) {
				...
				$.each(response, function(index, element) {
					...
					if(element.count > 0) {
						...
						classAMPM = (ea_settings["time_format"] == "am-pm") ? ' am-pm':'';
						next_element.append('<div class="time-disabled' + classAMPM  +'">' + element.show + '</div>');
					}
				});
		 ...
		 * @param $form
         */
		loadPreviusFormData: function($form) {
